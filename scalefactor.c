...
void scalefactor(TString mcName="/root_files_R48/SingleNeutrino_MC_R4.root", TString dataName="/root_files_R48/Legacy_BCD_R4.root", TString outName = "UL17_RunBCDEF_V1_", const double R=0.4, int pf_choice=1, int n1=10, int n2=52){
  ...
  ifstream mc_file(Form("./textfiles_Run3/indirectRho/R%i/",Rlabel) + pf_type + Form("/new-MinbiasXS/%sMC_L1RC_AK%iPF", outName.Data(), Rlabel) + pf_type + ".txt");
  ifstream data_file(Form("./textfiles_Run3/indirectRho/R%i/",Rlabel) + pf_type + Form("/new-MinbiasXS/%sDATA_L1RC_AK%iPF", outName.Data(), Rlabel) + pf_type + ".txt");
  ...
  ofstream writeFile(Form("./textfiles_Run3/scalefactor/R%i/",Rlabel) + pf_type + Form("/new-MinbiasXS/%sDataMcSF_L1RC_AK%iPF", outName.Data(), Rlabel) + pf_type + ".txt");
  ...
  for (int i=0; i<ETA_BINS; i++){
    ...
    //cout << "./text_files/scalefactor/R4/"+pf_type+Form("/%sscalefactor_AK%iPF",outName.Data(),Rlabel)+ pf_type + Form("_eta%4.3f.pdf", eta1[i]) << endl;
    c->Print(Form("./textfiles_Run3/scalefactor/R%i/",Rlabel) + pf_type + Form("/new-MinbiasXS/%sscalefactor_AK%iPF",outName.Data(),Rlabel)+ pf_type + Form("_eta%4.3f.pdf", eta1[i]) );
    delete h;
    delete c;
  }
  writeFile.close();

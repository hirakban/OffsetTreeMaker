...
void offsetpT(int n1=1, int n2=25, float topX=15, float topY=30){
  ...
  TFile* mcFile = TFile::Open( Form("/eos/uscms/store/group/lpcjme/L1Offset/UltraLegacy17_scalefactors/jetSort/Total_MC_UL2017%s_R%i_pTcut15GeV.root", run_name.Data(), Rlabel) );
  TFile* dataFile = TFile::Open( Form("/eos/uscms/store/group/lpcjme/L1Offset/UltraLegacy17_scalefactors/jetSort/Total_Data_UL2017%s_R%i_pTcut15GeV.root", run_name.Data(), Rlabel) );//_pTcut15GeV
  ...
  for (int n=0; n<nPoints; n++){
    if(hp.EqualTo(“h”)){
      ...
      mcIetas.push_back( ((TH1D*) mcFile->FindObjectAny(hieta))->SetName( hieta+“_mc") ));
      dataIetas.push_back( ((TH1D*) dataFile->FindObjectAny(hieta))->SetName( hieta+“_data") ));
    }
    ...
    if (pf_type.EqualTo("all") || pf_type.EqualTo("chs")){  
      ...
      if(hp.EqualTo(“p”)){
        ...
        mcProfiles.push_back( ((TH1D*) mcFile->FindObjectAny(hname))->SetName( ids[ids.size()-kk]+Form("%i_mc",n1+n) ) );
        dataProfiles.push_back( ((TH1D*) dataFile->FindObjectAny(hname))->SetName( ids[ids.size()-kk]+Form("%i_data",n1+n) ) );
      }
      for (int i=0; i<ids.size()-kk; i++){
        ...
        if(hp.EqualTo(“p”)){
          ...
          mcProfiles.back()->Add( ((TH1D*) mcFile->FindObjectAny(hname))->SetName( ids[i]+Form("%i_mc",n1+n) ) );
          dataProfiles.back()->Add( ((TH1D*) dataFile->FindObjectAny(hname))->SetName( ids[i]+Form("%i_data",n1+n) ) );
        }
      }
    } 
  ...
  ofstream writeMC("./plots/" + var_type + "/Run"+ run_name + Form("/R%i/",Rlabel) + pf_type + Form("/UL17_pTcut15GeVMEAN_new_Run%s_V1_MC_L1RC_AK%iPF", run_name.Data(), Rlabel) + pf_type + ".txt");
  ofstream writeData("./plots/" + var_type + "/Run"+ run_name + Form("/R%i/",Rlabel) + pf_type + Form("/UL17_pTcut15GeVMEAN_new_Run%s_V1_DATA_L1RC_AK%iPF",run_name.Data(), Rlabel) + pf_type + ".txt");
  ...
  for (int i=0; i<ETA_BINS; i++) {
    ...
    c->Print("./plots/" + var_type + "/Run"+ run_name + Form("/R%i/",Rlabel) + pf_type + "/" + pf_type + "_pT_" + var_type + Form("_pTcut15GeV_eta%4.3f", etabins[i]) + ".pdf");
    delete h;
    delete c;
  }
  writeMC.close();

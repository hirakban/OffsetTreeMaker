...
//      dataProfiles.push_back( v_depth ) ;
//      for (int i=1; i<all_depths; i++){
//        hname = pf_type + Form("_p_fract_depth%i_eta_%s%i_HCAL", i, var_type.Data(), n1+n) ;
//        cout << "hname = " << hname << endl;
//        v_depth = ((TProfile*) dataFile->FindObjectAny(hname)->ProjectionX(depths[i]+"MC",etabins[0],etabins[ETABINS]) );
//        v_alldepth->Add(v_depth);
//      }
//      mcProfiles.push_back( v_alldepth );
//--------------------
    else{
...
//--------Update for Run3-----------
  ofstream writeMC("./textfiles_Run3/" + var_type + Form("/R%i/",Rlabel) + pf_type + Form("/new-MinbiasXS/%sMC_L1RC_AK%iPF", outName.Data(), Rlabel) + pf_type + ".txt");
  ofstream writeData("./textfiles_Run3/" + var_type + Form("/R%i/",Rlabel) + pf_type + Form("/new-MinbiasXS/%sDATA_L1RC_AK%iPF",outName.Data(), Rlabel) + pf_type + ".txt");
  ...
  for (int i=0; i<ETA_BINS; i++) {
    ...
    for (int n=0; n<nPoints; n++){
      ...
      data_to_mc.push_back( data_y.back()/mc_y.back() );
    ...
    if (subplot == "pull"){
      ...
      text.DrawLatex(0.2, 0.88, "Data: Run2025Cv1v2");
...
//      c->Print("./textfiles_Run3/" + var_type + Form("/R%i/",Rlabel) + pf_type + "/depth/" + outName + pf_type + "_pT_" + var_type + Form("_eta%4.3f", etabins[i]) + ".pdf");
      ...
      c->Print("./textfiles_Run3/" + var_type + Form("/R%i/",Rlabel) + pf_type + "/new-MinbiasXS/" + outName + pf_type + "_pT_" + var_type + Form("_eta%4.3f", etabins[i]) + ".pdf");
